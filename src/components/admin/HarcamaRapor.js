...
const HarcamaRapor = () => {
  ...
  useEffect(() => {
    const fetchFiles = async () => {
      try {
        const response = await axios.get('http://92.112.193.8:5018/api/harcama-files');
  ...
  const handleFileChange = async (event) => {
    ...
    if (filename) {
      try {
        const response = await axios.get(`http://92.112.193.8:5018/api/harcama-excel/${filename}`);
