...
<<<<<<< HEAD
      ...
      const formattedResults = results.map(item => {
        return {
          ...item,
          date: new Date(item.date).toLocaleDateString('en-CA') // Tarihi YYYY-MM-DD formatında döndürüyoruz
        };
      });
...
app.get('/api/monthly-average-cash', (req, res) => {
  const { managerId } = req.query;
  const currentMonth = dayjs().month() + 1;
  const currentYear = dayjs().year();
  
  const query = `
    SELECT AVG(pos_revenue) as averageCash 
    FROM cash_operations 
    WHERE manager_id = ? AND MONTH(date) = ? AND YEAR(date) = ?`;
  
  db.query(query, [managerId, currentMonth, currentYear], (err, results) => {
    if (err) {
      console.error('Error fetching monthly average cash:', err);
      res.status(500).send('Error fetching monthly average cash');
    } else {
      res.json(results[0]);
    }
  });
